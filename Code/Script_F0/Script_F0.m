...
po_WOA_BWOA = zeros(length(F0), noRealizations);
su_WOA_BWOA = zeros(length(F0), noRealizations);
po_PSO_BWOA = zeros(length(F0), noRealizations);
su_PSO_BWOA = zeros(length(F0), noRealizations);
po_IWOA_BWOA = zeros(length(F0), noRealizations);
...
%             function in ..\WOA_voronoi
        ...
        % offloading vector
        A_WOA_BWOA   = sum(sum(leader_pos_bwoa, 2),3); % N x 1 matrix
        off_users_no = sum(A_WOA_BWOA);
