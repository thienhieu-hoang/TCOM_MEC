...
% leader_pos_tpc    == N x 1 matrix

function [leader_score_bwoa, leader_pos_bwoa, leader_pos_tpc, conver_curve, conver_curve_tpc, no_TPC_run, time] = with_BWOA2(functionName, TPC, doTol, noSearchAgents, noUsers, noSubcs, noBSs, UE_BS, maxIter, fobj_bwoa, lb_woa, ub_woa, fobj_woa, theta, eta, W, h2h, n0, p_min, p_max, nu, Adetermined)
...
tic 
	...
	% leader_score_woa 
	leader_pos_tpc = zeros(noUsers, 1); 


	% loop counter
	%doTol = 1; % = doTol == 0 to check all 150 iterations
	delta = 1e-5; 
	flag = 0; 
	
	conver_curve = zeros(1, maxIter);
    conver_curve_tpc = zeros(1, maxIter); 
...
%            fprintf('SearchAgent no %i/%i\n', nSA, noSearchAgents)
 ...
 %           fprintf('old_time for condition 2: %i', t1);
            ...
            no_TPC_run = no_TPC_run + 1; 
            
            % to compare WOA, IWOA, PSO
            switch TPC
                case {'WOA'}
                    [TPC_rs, pos_tpc, ~] = WOA(noSearchAgents, ...
				        noUsers, maxIter, lb_woa, ub_woa, fobj_woa, positions(:,:,:,nSA));
                case {'IWOA'}
                    [TPC_rs, pos_tpc, ~] = IWOA(noSearchAgents, ...
				        noUsers, maxIter, lb_woa, ub_woa, fobj_woa, positions(:,:,:,nSA));
                case {'IWOAmatrix'}
                    [TPC_rs, pos_tpc, ~] = IWOA_matrix(noSearchAgents, ...
				        noUsers, maxIter, lb_woa, ub_woa, fobj_woa, positions(:,:,:,nSA));
                case {'PSO'}
                    [TPC_rs, pos_tpc, ~] = PSO(noSearchAgents, ...
				        noUsers, maxIter, lb_woa, ub_woa, fobj_woa, positions(:,:,:,nSA));
                case {'compare'}
                    [TPC_rs, pos_tpc, WOA_curve] = WOA(noSearchAgents, ...
				        noUsers, maxIter, lb_woa, ub_woa, fobj_woa, positions(:,:,:,nSA));
                    [~, ~, IWOA_curve] = IWOA(noSearchAgents, ...
				        noUsers, maxIter, lb_woa, ub_woa, fobj_woa, positions(:,:,:,nSA));
                    [~, ~, PSO_curve] = PSO(noSearchAgents, ...
				        noUsers, maxIter, lb_woa, ub_woa, fobj_woa, positions(:,:,:,nSA));
%                     [~, ~, IWOAmatrix_curve] = IWOA_matrix(noSearchAgents, ...
% 				        noUsers, maxIter, lb_woa, ub_woa, fobj_woa, positions(:,:,:,nSA));
                    plot(1:length(WOA_curve), WOA_curve);
                    hold on;
                    plot(1:length(IWOA_curve), IWOA_curve);
                    hold on;
                    plot(1:length(PSO_curve), PSO_curve);
%                     hold on;
%                     plot(1:length(IWOAmatrix_curve), IWOAmatrix_curve);
            ...
            fitness = fitbwoa - TPC_rs; % double
            % make a break point here to see the comparison figure/ 
            % Can run another iteration to see the later convergence

			% update the leader
			if fitness >= leader_score_bwoa
				leader_score_bwoa = fitness; 
				leader_pos_bwoa   = positions(:, :, :, nSA); 
				leader_pos_tpc 	  = pos_tpc; 
				leader_score_TPC  = TPC_rs; 
                whale_no          = nSA;
            end 
...
%         fprintf('iter %i\n', iter);
		...
		conver_curve_tpc(iter) = leader_score_TPC;
...
%		fprintf('iter:%i/%i, leader_score_woa:%i, leader_score_bwoa: %i\n', iter, maxIter, leader_score_woa, leader_score_bwoa)
		...
		if flag == 200
			...
			conver_curve_tpc = conver_curve_tpc(1:iter); 
			break; 
		end 
	end
	toc
