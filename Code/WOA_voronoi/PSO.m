...
function [leaderScore, leaderPos, convergenceCurve] = PSO(noSearchAgents, noUsers, MaxIt, lb, ub, fobj, X) 
	% Input:
	% noSearchAgents: number of particles
...
%     noSearchAgents = 100;   % nPop=100;        % Population Size (Swarm Size)
    ...
    % Velocity Limits
    VelMax = 0.1*(ub - lb); % N x 1 
    VelMin = - VelMax;      % N x 1 
    

	% ======================== Initialization =================

    empty_particle.Position      = []; % N x 1 matrix
    empty_particle.Cost          = []; 
    empty_particle.Velocity      = []; % N x 1 matrix
    ...
    for i = 1 : noSearchAgents
        ...
        % Initialize Position
        particle(i).Position = 0.5*rand(noUsers, 1).*(ub - lb)/100 + lb;
                              % N x 1 matrix

        % Initialize Velocity
        particle(i).Velocity = zeros(noUsers,1); % N x 1
            
        % Evaluation
        particle(i).Cost    = fobj(particle(i).Position, X);            
        
  % ??      % Update Personal Best ??
        particle(i).Best.Position = particle(i).Position;   % N x 1
    ...
    while (it < MaxIt && flag<10)
        ...
        for i = 1 : noSearchAgents
            ...
            particle(i).Position = max(particle(i).Position, lb); % N x M
